...
export class UserAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const session = await this.prisma.sessionByToken(sha256(rawToken));
    if (!session)
      throw new UnauthorizedException({
    ...
    if (
      ...
      throw new UnauthorizedException({
        ...
        message: "Temporary access expired. Use password recovery.",
      });
    ...
    const recoveryRoute =
      ["session", "logout", "changePassword"].includes(
    ...
    if (!access.allowed && !recoveryRoute)
      throw new ForbiddenException({
        ...
        message: isOwner
          ? access.reason
          : "Business access is restricted. Contact your owner.",
      });
...
export class PermissionGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (
      ...
      !context.getHandler().name.includes("changePassword")
    ) {
      throw new ForbiddenException({
...
export class PlatformAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const rawToken = request.cookies?.[
      `${this.config.get<string>("SESSION_COOKIE_NAME", "cafe_pos_session")}_platform`
    ] as string | undefined;
    if (!rawToken)
      throw new UnauthorizedException({
        code: "AUTH_REQUIRED",
    ...
    const session = await this.prisma.platform.platformSession.findFirst({
      where: {
      ...
      select: {
        id: true,
        csrfHash: true,
        platformAdmin: { select: { id: true, name: true } },
      },
    });
    if (!session)
      throw new UnauthorizedException({
